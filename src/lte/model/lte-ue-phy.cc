...
void
LteUePhy::SetStreamForMeasuringSinr(std::ofstream* sr){
	stream = sr;
}
  ...
  if (m_rsrpSinrSampleCounter==m_rsrpSinrSamplePeriod)
    {
      ...
      double avSinr_2 = Sum(sinr)/(sinr.GetSpectrumModel()->GetNumBands());
      m_reportCurrentCellRsrpSinrTrace (m_cellId, m_rnti, rsrp, avSinr);
   *stream << Simulator::Now().GetSeconds() <<"\t"<<10*log10(avSinr_2)<<std::endl; //sjkang
      m_rsrpSinrSampleCounter = 0;
    }
      ...
      if (pb)
      ...
      else
        {
          ...
          if (ctrlMsg.size ()>0)
           ...
           //   std::cout<< "sending phy data---------------" <<std::endl;
              m_uplinkSpectrumPhy->StartTxDataFrame (pb, ctrlMsg, UL_DATA_DURATION);
            }
          else
            {
