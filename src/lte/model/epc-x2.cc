...
EpcX2::DoDispose (void)
{
 ...
 // std::cout << "epc_x2 disposed -->" << std::endl;
...
void
EpcX2::SetEpcX2PdcpUser(EpcX2PdcpUser *s){ //sjkang 1114
	m_x2PdcpUser =s ;
}
void
...
void
EpcX2::SetMcEpcX2RlcUser_2 (uint32_t teid, EpcX2RlcUser* rlcUser) //sjkang1016
{
  // TODO it may change (for the same teid) on handover between secondary cells, as in LteEnbRrc::RecvRlcSetupRequest
  //NS_ASSERT_MSG(m_x2RlcUserMap.find(teid) == m_x2RlcUserMap.end(), "Teid " << teid
  //  << " is already setup\n");
...
void
EpcX2::SetMcEpcX2PdcpUser_2 (uint32_t teid, EpcX2PdcpUser* pdcpUser) //sjkang1016
{
  // TODO it may change (for the same teid) on handover between secondary cells, as in LteEnbRrc::RecvRlcSetupRequest
  //NS_ASSERT_MSG(m_x2PdcpUserMap.find(teid) == m_x2PdcpUserMap.end(), "Teid " << teid
  //  << " is already setup\n");
...
EpcX2::RecvFromX2cSocket (Ptr<Socket> socket)
{
  ...
  if (procedureCode == EpcX2Header::HandoverPreparation)
    {
      ...
      //    params.lteCellId = x2HoReqHeader.GetLteCellId(); //sjkang0416
      ...
      else if (messageType == EpcX2Header::SuccesfulOutcomToLte)
             {
               NS_LOG_INFO ("Recv X2 message: HANDOVER REQUEST ACK To LTE");
               ...
               m_x2SapUser->RecvHandoverRequestAckToLte(params);
             }
  ...
  else if(procedureCode == EpcX2Header::SendingAssistantInformation){ //sjkang1114

    	if (messageType == EpcX2Header::McAssistantInfoForwarding){ //sjkang1114
    		  NS_LOG_INFO ("Recv X2 message: from Socket");
    		EpcX2AssistantInfoHeader epcX2AssistantHeader;
    		packet->RemoveHeader(epcX2AssistantHeader);
    		EpcX2SapUser::AssistantInformationForSplitting params;
    		params.Tx_On_Q_Size = epcX2AssistantHeader.GetTxonQueue();
    		params.Txed_Q_Delay =epcX2AssistantHeader.GetTxedQueue();
    		params.Re_TX_Q_Size =epcX2AssistantHeader.GetRetxQueue();
    		params.Tx_On_Q_Delay = epcX2AssistantHeader.GetTxonQueingDelay();
    		params.Re_Tx_Q_Delay = epcX2AssistantHeader.GetReTxQueuingDelay();
    		params.sourceCellId = epcX2AssistantHeader.GetSourceCellId();
    		params.rnti = epcX2AssistantHeader.GetRnti();
    		params.drbId = epcX2AssistantHeader.GetDrbId();
  ...
  else if (procedureCode == EpcX2Header::BufferDuplication){ //sjkang0802
	  if (messageType == EpcX2Header::DuplicationRlcBuffer){
		//  std::cout << " eNB receives buffer duplication message "<< std::endl;
		  EpcX2DuplicationRlcBufferHeader duplHeader;
		  packet->RemoveHeader(duplHeader);
		  EpcX2SapProvider::SendBufferDuplicationMessage params;
...
EpcX2::RecvFromX2uSocket (Ptr<Socket> socket)
{
  ...
  if(m_teidToBeForwardedMap.find(params.gtpTeid) == m_teidToBeForwardedMap.end())
  {
    if(gtpu.GetMessageType() == EpcX2Header::McForwardDownlinkData)
    {
      ...
      EpcX2RlcUser* user_2 = m_x2RlcUserMap_2.find(params.gtpTeid)->second; //sjkang1016
...
EpcX2::DoSendHandoverRequest (EpcX2SapProvider::HandoverRequestParams params)
{
 ...
 //x2HoReqHeader.SetLteCellId(params.lteCellId); //sjkang0416
...
void
EpcX2::DoSendHandoverRequestAckToLte (EpcX2SapProvider::HandoverRequestAckParams params)
{
  NS_LOG_FUNCTION (this);
 //std::cout << "-------------- X2 will send handover request ack to Lte eNB -----------" <<std::endl;
  NS_LOG_LOGIC ("oldEnbUeX2apId = " << params.oldEnbUeX2apId);
  NS_LOG_LOGIC ("newEnbUeX2apId = " << params.newEnbUeX2apId);
  NS_LOG_LOGIC ("sourceCellId = " << params.sourceCellId);
  NS_LOG_LOGIC ("targetCellId = " << params.targetCellId);
  ...
  EpcX2Header x2Header;
  x2Header.SetMessageType (EpcX2Header::SuccesfulOutcomToLte);
  x2Header.SetProcedureCode (EpcX2Header::HandoverPreparation);
  x2Header.SetLengthOfIes (x2HoAckHeader.GetLengthOfIes ());
  x2Header.SetNumberOfIes (x2HoAckHeader.GetNumberOfIes ());
...
EpcX2::DoSendMcPdcpPdu(EpcX2Sap::UeDataParams params)
{
 ...
 // blocked by gsoul for tag error
 // EpcX2Tag tag (Simulator::Now()); //sjkang1221
 // packet->AddPacketTag (tag);  

  NS_LOG_INFO ("Forward MC UE DATA through X2 interface");
  sourceSocket->SendTo (packet, 0, InetSocketAddress (targetIpAddr, m_x2uUdpPort));  /// should be transmitted to EpcX2:RecvFromX2Socket
}
...
void
EpcX2::DoReceiveAssistantInformation(EpcX2Sap::AssistantInformationForSplitting params){ //sjkang
...
NS_LOG_INFO (this);
 EpcX2AssistantInfoHeader assistantInfoHeader;
 ...
 epcX2Header.SetMessageType(EpcX2Header::McAssistantInfoForwarding);
 epcX2Header.SetProcedureCode(EpcX2Header::SendingAssistantInformation);
 epcX2Header.SetLengthOfIes(assistantInfoHeader.GetSerializedSize());
 epcX2Header.SetNumberOfIes(assistantInfoHeader.GetNumbefOfIes());

 Ptr<Packet> packet =Create<Packet>();
 packet->AddHeader(assistantInfoHeader);
 packet->AddHeader(epcX2Header);
...
void
EpcX2::DoDuplicateRlcBuffer(EpcX2SapProvider::SendBufferDuplicationMessage params){
	 Ptr<X2IfaceInfo> socketInfo = m_x2InterfaceSockets [params.targetCellId];
	  Ptr<Socket> sourceSocket = socketInfo->m_localCtrlPlaneSocket;
	  Ipv4Address targetIpAddr = socketInfo->m_remoteIpAddr;
	  ...
	  duplHeader.SetOption(params.option);
	  	  ...
	  	  EpcX2Header x2Header;
	  x2Header.SetProcedureCode (EpcX2Header::BufferDuplication);
	  x2Header.SetMessageType (EpcX2Header::DuplicationRlcBuffer);
