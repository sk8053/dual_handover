...
LteRlcAm::LteRlcAm ()
{
  ...
  lastSumPacketSize=0;
  TotalTime=0;
  TxOn_QueingDelay =0 ; //sjkang
  ReTx_QueingDelay =0 ; //sjkang
  //std::cout<<m_rlcSapUser <<std::endl;
}
...
LteRlcAm::DoDispose ()
{
  ...
  m_sendLteAssistantInfo.Cancel();
  m_reorderingQueueStatistic.Cancel();
  m_calculatedTput.Cancel();
...
LteRlcAm::DoTransmitPdcpPdu (Ptr<Packet> p)
{
  ...
  if (m_epcX2RlcProvider !=0 && reporting_Start==0){ //sjkang1114
       ...
       std::cout << "will send assistant info from LteRlcAm" <<std::endl;
       RecordingQueueStatistics();
       Simulator::Schedule(MilliSeconds(10),&LteRlcAm::SendAssistantInformation,this, info);
  ...
  // std::cout << m_txonQueue->GetNPackets() <<std::endl;
  }
...
{
  NS_LOG_INFO(this <<"\t"<< m_rnti <<"\t"<< (uint32_t) m_lcid<<"\t" << p->GetSize ());
...
LteRlcAm::DoReportBufferStatus (void)
{
 ...
 // m_txBufferSize = queueSize; //sjkang1114
  TxOn_QueingDelay = r.txQueueHolDelay; //sjkang1114
   ReTx_QueingDelay = r.retxQueueHolDelay; //sjkang0104
...
void
LteRlcAm::SendAssistantInformation(EpcX2Sap::AssistantInformationForSplitting info){
	info.targetCellId = m_ueDataParams.targetCellId;
	info.sourceCellId = m_ueDataParams.sourceCellId;
    info.Tx_On_Q_Size= m_txonBufferSize + m_txedBufferSize;
    info.Re_TX_Q_Size= m_retxBufferSize;
    info.Txed_Q_Size = m_txedBufferSize;
    info.Tx_On_Q_Delay = TxOn_QueingDelay;
    info.Re_Tx_Q_Delay = ReTx_QueingDelay;
    info.rnti = m_rnti;
    info.drbId = m_drbId;
    m_epcX2RlcProvider->ReceiveAssistantInformation(info);

if (!m_sendAssistatInfo.IsRunning())
	     m_sendAssistatInfo = Simulator::Schedule(MilliSeconds(1),&LteRlcAm::SendAssistantInformation,this, info);

}

void
LteRlcAm::SendLteAssistantInformation(EpcX2Sap::AssistantInformationForSplitting info){//sjkang1114
	info.targetCellId = m_ueDataParams.targetCellId;
	info.sourceCellId = m_ueDataParams.sourceCellId;
    info.Tx_On_Q_Size= m_txonBufferSize + m_txedBufferSize;
    info.Re_TX_Q_Size= m_retxBufferSize;
    ...
    info.rnti = m_rnti;
    info.drbId = m_drbId;
//std::cout<<"LTE at Rlc Am  " <<(double) (info.Txed_Q_Delay+info.Tx_On_Q_Delay)/10e3 <<"s"<<std::endl;
  m_rlcSapUser->SendLteAssi(info);

	//}
  if(!m_sendLteAssistantInfo.IsRunning())
	    m_sendLteAssistantInfo = Simulator::Schedule(MilliSeconds(1),&LteRlcAm::SendLteAssistantInformation,this, info);
}
...
TotalPackets =sumPacketSize;
TotalTime += 0.1;
/*if (now.GetSeconds()>= m_simulationTime.GetSeconds()+1.0 && countAtRlc == 0 ){
	outputAtRlc<< cur << "\t";
	countAtRlc =1;
	t_Address =this;
	throughputAtSenb = cur;
}
else if (now.GetSeconds() >=m_simulationTime.GetSeconds()+1.0 && countAtRlc ==1 && t_Address != this ){
	outputAtRlc<< cur << "\t" <<std::endl;
	countAtRlc =2;
	outputAtRlc.close();
	set_1<<"\t" << cur << "\t" << throughputAtSenb << std::endl;
    set_1.close();
} */
//std::cout << sumPacketSize << std::endl;
  *stream << now.GetSeconds () << "\t" << cur << "\t"<<(double)((TotalPackets*8)/TotalTime)/1e6 <<std::endl;
  lastSumPacketSize = sumPacketSize;
  if(!m_calculatedTput.IsRunning())
 m_calculatedTput = Simulator::Schedule (MilliSeconds (100), &LteRlcAm::CalculatePathThroughput, this, stream);
}
void
LteRlcAm::RecordingQueueStatistics(){//sjkang1116

	*measuringQusizeQueueDelayStream <<Simulator::Now().GetSeconds()<<"\t"<<m_txonBufferSize+m_txedBufferSize<< "\t"
			<< m_retxBufferSize<<"\t" << TxOn_QueingDelay/10e3<<"\t"<<ReTx_QueingDelay/10e3<< std::endl;
  if(!m_reorderingQueueStatistic.IsRunning())
	m_reorderingQueueStatistic = Simulator::Schedule(MilliSeconds(1.0),&LteRlcAm::RecordingQueueStatistics,this);
}
void
LteRlcAm::DoRequestAssistantInfo(){

	             EpcX2Sap::AssistantInformationForSplitting info;
	            std::cout << "will send LTE assistant info from LteRlcAm" <<std::endl;
	            RecordingQueueStatistics();
	           Simulator::Schedule(MilliSeconds(10),&LteRlcAm::SendLteAssistantInformation,this, info);
