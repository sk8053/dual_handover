...
namespace ns3 {
	...
	void
		McUePdcp::DoTransmitPdcpSdu (Ptr<Packet> p)
		{
			...
			LteRlcSapProvider::TransmitPdcpPduParameters params, params_2;
			params.rnti = m_rnti; params_2.rnti = m_rnti;
			params.lcid = m_lcid; params_2.lcid = m_lcid;
			params.pdcpPdu = p; params_2.pdcpPdu = p->Copy();

			// WARN TODO hack: always use LTE for uplink (i.e. TCP acks)
			//m_useMmWaveConnection=true;  // will transmit uplink data to LTE cell
			//m_alwaysLteUplink = false;
			...
			else if (m_useMmWaveConnection)
			{
				...
				m_mmWaveRlcSapProvider_2->TransmitPdcpPdu (params_2);//sjkang1110
			}
			else
			{
	...
	void
		McUePdcp::BufferingAndReordering_New(Ptr<Packet> p){ // sjkang
			...
			std::map<uint64_t, LtePdcpSapUser::ReceivePdcpSduParameters>::iterator iter;
			...
			if (outOfDelivery){
				std::map<uint64_t, LtePdcpSapUser::ReceivePdcpSduParameters> ::iterator it;
