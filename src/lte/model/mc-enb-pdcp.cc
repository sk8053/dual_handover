...
namespace ns3 {
		...
		McEnbPdcp::GetTypeId (void)
		{
			static TypeId tid = TypeId ("ns3::McEnbPdcp")
				...
				.AddAttribute("enableLteMmWaveDC", "this value means if Lte - mmWave DC is enable or not", BooleanValue(false),
						...
						MakeBooleanChecker())
				.AddAttribute("EnableDuplication", 
					      "Enabling Duplication mode",
					       BooleanValue(true), 
					       MakeBooleanAccessor(&McEnbPdcp::m_isEnableDuplicate),
					       MakeBooleanChecker())//added by gsoul 180920
				...
				case 2: //alternative
					{
						count ++;
						if (count % 2 == 0){
							NS_LOG_UNCOND(targetCellId_1<<" sends "<<count);
							return targetCellId_1;
						}
						else{	
							NS_LOG_UNCOND(targetCellId_2<<" sends "<<count);
							return targetCellId_2;
						}
	...
	void
		McEnbPdcp::DoReceiveAssistantInformation(EpcX2Sap::AssistantInformationForSplitting info){
			...
			//	std::cout <<info.sourceCellId << "\t" <<  q_Size[info.sourceCellId] << std::endl;
