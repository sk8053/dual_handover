...
uint8_t
LtePdcpHeader::GetSourceCellId() const
{
	return sourceCellId;
}
void
LtePdcpHeader::SetSourceCellId (uint8_t sourceCellId){
...
void LtePdcpHeader::Print (std::ostream &os)  const
{
  ...
  os << " Source Cell Id = " <<(int)sourceCellId;
}
...
void LtePdcpHeader::Serialize (Buffer::Iterator start) const
{
  ...
  i.WriteU8((uint8_t)sourceCellId); //sjkang1116
}
