...
namespace ns3 {
		...
		m_expectedSeqNumber (0)
	{
		...
		lastSumPacketSize =0;
		TotalTime =0.0;
		this->isEnbaleMeasuring=false; //sjkang
	...
	{
		NS_LOG_FUNCTION (this);
		//  std::cout<< "RlcUmDestried  " << this << std::endl;
	}
	...
	void
		LteRlcUm::DoDispose ()
		{
			...
			m_rbsTimer.Cancel ();
			m_Tput_Calculator.Cancel();
			calculateThroughput.Cancel();
			stream->close();
	...
	std::ofstream rlcDelay ("rlcDelay.txt");
	void
		LteRlcUm::DoReceivePdu (Ptr<Packet> p)
		{
			NS_LOG_FUNCTION (this << m_rnti << (uint32_t) m_lcid << p->GetSize ());

			if(!isEnbaleMeasuring)
			{
				if (!m_Tput_Calculator.IsRunning()){
					//  this->CalculatePathThroughput(this->stream);
			...
			/*if (m_epcX2RlcProvider !=0){
			  ...
			  DoSendAssistantInformation(info);
			  } */
			//std::cout << "----------------------received data   " <<std::endl;
			//}
			//std::cout<< "RlcUm receive data " << std::endl;
			// Receiver timestamp
			...
			if (p->FindFirstMatchingByteTag (rlcTag))
			{
				...
				rlcDelay<<m_rnti<<"\t"<<Simulator::Now().GetSeconds()<<"\t"<<delay<<std::endl;
			}
			m_rxPdu (m_rnti, m_lcid, p->GetSize (), delay.GetNanoSeconds ());
			//	rlcDelay <<this<< " \t" <<  Simulator::Now().GetSeconds()<<"\t"<< m_rxBuffer.size()<<std::endl;
			...
			//rlcDelay <<m_rnti<< " \t" <<  Simulator::Now().GetSeconds()<<"\t"<< m_rxBuffer.size()<<"\t" << seqNumber<<std::endl;

			//std::cout<<"RX   " << this<< "\t"<<Simulator::Now().GetSeconds()<<"\t" << seqNumber <<"\t" << p->GetSize()<< std::endl;
			// 5.1.2.2.1 General
...
LteRlcUm::CalculateThroughput(){
	...
	*stream << now.GetSeconds () << "\t" << cur << "\t"<<(double)((TotalPackets*8)/TotalTime)/1e6 <<std::endl;
	lastSumPacketSize = sumPacketSize;
	if(!calculateThroughput.IsRunning())
		calculateThroughput = Simulator::Schedule (MilliSeconds (100), &LteRlcUm::CalculatePathThroughput,this , stream);
}
void
