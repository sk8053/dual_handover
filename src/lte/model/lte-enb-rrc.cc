...
void
UeManager::ForwardRlcBuffers(Ptr<LteRlc> rlc, Ptr<LtePdcp> pdcp, uint32_t gtpTeid,
		bool mcLteToMmWaveForwarding, bool mcMmToMmWaveForwarding, uint8_t bid)
{
  NS_LOG_FUNCTION(this );
...
std::ofstream bufferforward ("bufferfowrding_data.txt");
void
UeManager::ForwardRlcBuffers(Ptr<LteRlc> rlc, Ptr<LtePdcp> pdcp, uint32_t gtpTeid,
		bool mcLteToMmWaveForwarding, bool mcMmToMmWaveForwarding, uint8_t bid, bool copyMode)
{
  NS_LOG_FUNCTION(this );
 	// RlcBuffers forwarding only for RlcAm bearers.
  NS_LOG_FUNCTION(this );
    ...
    if (0 != rlc->GetObject<LteRlcAm> ())
    {
          ...
          m_x2forwardingBufferSize =   txonBufferSize;
 	 	 ...
 	 	 std::cout<<Simulator::Now().GetSeconds()<<" ------------------  Rlc buffer forwarding ------------------ " << m_x2forwardingBuffer.size()<<"\t"<< m_x2forwardingBufferSize <<std::endl;

    }
   if (0 != rlc->GetObject<LteRlcUmLowLat> ())
  {
    //Copy lte-rlc-um-low-lat.m_txOnBuffer to X2 forwarding buffer.
    NS_LOG_DEBUG(this << " forwarding txonBuffer from RLC UM " << m_rnti);
    ...
    std::cout<<"------------------  Copy and Forward RLC Buffer ------------------ " << m_x2forwardingBuffer.size()<<"\t"<< m_x2forwardingBufferSize <<std::endl;
    NS_LOG_UNCOND("Forward Rlc Buffer to target cell RLC in HO  to " << m_targetCellId << "\t "<<" from " <<
          		   m_rrc->m_cellId);
 ...
 // uint16_t cc = 0;
  ...
  // iter = m_x2forwardingBuffer.begin();
 	// if (m_x2forwardingBuffer.size() > 10)
   	 //  ++iter;
   //while ( iter!=m_x2forwardingBuffer.end())
 //while (!m_x2forwardingBuffer.empty())
   ...
   // Ptr<Packet> rlcSdu =  (*(m_x2forwardingBuffer.begin()))->Copy();
    ...
    if(rlcSdu->GetSize() >= 3)
    {
      ...
      { //ignore control SDU.
        ...
        //   NS_LOG_UNCOND("Forward to target cell RLC in HO  to " << m_targetCellId << "\t "<<m_secondMmWaveCellID << " from " <<
            m_rrc->m_x2SapProvider->ForwardRlcPdu (params);
            bufferforward <<Simulator::Now().GetSeconds() <<"\t" <<params.ueData->GetSize ()
				<<std::endl;
            NS_LOG_LOGIC ("sourceCellId = " << params.sourceCellId);
            NS_LOG_LOGIC ("targetCellId = " << params.targetCellId);
            NS_LOG_LOGIC("gtpTeid = " << params.gtpTeid);
    ...
    m_x2forwardingBufferSize -= (*(m_x2forwardingBuffer.begin()))->GetSize();
//    m_x2forwardingBuffer.erase (m_x2forwardingBuffer.begin());
    //m_x2forwardingBuffer.erase (iter);
    }
   // iter++;
    //NS_LOG_UNCOND(this << " After forwarding: buffer size = " << m_x2forwardingBufferSize );
  }
    ...
    case HANDOVER_JOINING:
      {
        ...
        //  std::cout << GetIsMc() << "\t" << GetIsMc_2() << std::endl;
          for ( std::map <uint8_t, Ptr<RlcBearerInfo> >::iterator rlcIt = m_rlcMap.begin ();
        	  ...
        	  EpsBearer bearer;
        	     TypeId rlcTypeId = m_rrc->GetRlcType (bearer); // actually, this doesn't depend on bearer
        	  if (rlcTypeId == LteRlcUmLowLat::GetTypeId()){
        		  if (GetIsMc())
        			  rlcIt->second->m_rlc->GetObject<LteRlcUmLowLat>()->SetRlcSN();
        		  else if (GetIsMc_2())
        			  rlcIt->second->m_rlc_2->GetObject<LteRlcUmLowLat>()->SetRlcSN();
        	  }else if (rlcTypeId == LteRlcAm::GetTypeId()){
        		  if (GetIsMc())
        		       rlcIt->second->m_rlc->GetObject<LteRlcAm>()->SetRlcSN();
        		   else if (GetIsMc_2())
        		       rlcIt->second->m_rlc_2->GetObject<LteRlcAm>()->SetRlcSN();
...
UeManager::RecvSecondaryCellHandoverCompleted(EpcX2Sap::SecondaryHandoverCompletedParams params) //LTE eNB receives Rrc Connection Reconfiguration Completed message
{
	...
	//  std::cout <<"sjkang0709 log message ----> "<<  params.cellId << "\t"<< params.oldEnbUeX2apId << std::endl;
	  ...
	  m_rrc-> ImsiTosecondCellRnti[m_imsi] = m_mmWaveRnti_73;
	  }
	  SetIsMc(params.isMc);
	  SetIsMc_2(params.isMc_2);
	//  std::cout << GetIsMc() << "\t" << GetIsMc_2() << std::endl;
     ...
     ++it)
  {
    if(!(it->second->m_isMc_2) || (it->second->m_isMc_2 && m_rrc->m_lastMmWaveCell.find(m_imsi)->second != m_mmWaveCellId_first))
    {
      ...
      if (pdcp != 0)
      {
       ...
       //	 std::cout << "Path Switch To eNB : " << m_mmWaveCellId_first << std::endl;
       	 	 m_rrc->To_28[m_imsi] = false; m_rrc->To_73[m_imsi] = true;
        }
        else if (GetIsMc()){
        	pdcp->SetMmWaveRnti(m_mmWaveRnti);
        // 	 std::cout << "Path Switch To eNB : " << m_mmWaveCellId << std::endl;
        	 pdcp->SetTargetCellIds(m_mmWaveCellId, m_mmWaveCellId,m_rrc->GetCellId());//sjkang0322
        	 m_rrc->To_28[m_imsi] = true; m_rrc->To_73[m_imsi] = false;
        ...
        m_rrc->m_lastMmWaveCell_2[m_imsi] = m_mmWaveCellId; //sjkang0709 //this is second cellID // 28G
        m_rrc->m_mmWaveCellSetupCompleted[m_imsi] = true;
...
UeManager::SendRrcConnectionSwitch(bool useMmWaveConnection)
{
  ...
  for (std::map <uint8_t, Ptr<LteDataRadioBearerInfo> >::iterator it =  m_drbMap.begin ();
     ...
     ++it)
  {
    if(it->second->m_isMc or it->second->m_isMc_2)
    {
      ...
      if(pdcp != 0)
      {
        ...
        else if(!m_rrc->m_ismmWave && !useMmWaveConnection && !GetIsMc_2())//sjkang
        {
          ...
          EpcX2SapProvider::SwitchConnectionParams params;
        	 ...
        	 // params.mmWaveRnti = m_rrc->ImsiTosecondCellRnti[m_imsi];

          params.mmWaveCellId = m_mmWaveCellId; // 28GHz
          params.useMmWaveConnection = useMmWaveConnection;
          params.drbid = it->first;
          //mmWaveRnti_73
      //    params_2.mmWaveRnti = m_rrc->ImsiTosecondCellRnti[m_imsi];
     //     params_2.mmWaveCellId = m_mmWaveCellId_first; // 73GHz
    //     params_2.useMmWaveConnection = useMmWaveConnection;
     //    params_2.drbid = it->first;

         std::cout << "send Switch message to LTE eNB 1) mmWaveRnti "<< m_mmWaveRnti <<
        		 "\t"<< "2) mmWaveRnti_73 " << m_mmWaveRnti_73 << std::endl;
          m_rrc->m_x2SapProvider->SendSwitchConnectionToMmWave(params);
       //   m_rrc->m_x2SapProvider->SendSwitchConnectionToMmWave(params_2);
         }
...
void
UeManager::SendRrcConnectionSwitch_2(bool useMmWaveConnection){
	std::cout << "Send Rrc Connection Switch to mmWave cell: " <<GetImsi( )<< std::endl;
  LteRrcSap::RrcConnectionSwitch msg;
  msg.rrcTransactionIdentifier = GetNewRrcTransactionIdentifier();
  std::vector<uint8_t> drbidVector;
     ...
     ++it)
  {
    if(it->second->m_isMc or it->second->m_isMc_2)
    {
      ...
      if(pdcp != 0)
      {
        ...
        {
          // switch from mmWave to LTE: it will be the mmWave cell that forwards the data back to LTE
          EpcX2SapProvider::SwitchConnectionParams  params_2;
        	 // params.mmWaveRnti = m_mmWaveRnti;
        	 // params.mmWaveRnti = m_rrc->ImsiTosecondCellRnti[m_imsi];
...
{
  std::cout << "Recv Connection Switch to Mmwave ---->" << GetImsi() << std::endl;
	NS_LOG_FUNCTION(this);
  ...
  if(!useMmWaveConnection)
  {
    ...
    if (GetIsMc())
    	...
    	ForwardRlcBuffers(m_rlcMap.find(drbid)->second->m_rlc_2, 0, m_rlcMap.find(drbid)->second->gtpTeid, 0, 0, 0);
   ...
   // else if (GetIsMc_2())
    	m_rlcMap.find(drbid)->second->m_rlc_2 = 0;
    ...
    rlc->SetImsi (m_imsi); //sjkang
    std::ostringstream fileName;
    	if(rlcTypeId ==LteRlcAm::GetTypeId())  //sjkang0827
    		fileName<<"rlcAmTx_queue_menb_ue"<<m_rnti<<"_bearer"<< (uint16_t)(drbid) << ".txt";
       else if (rlcTypeId == LteRlcUmLowLat::GetTypeId())
           fileName<<"rlcUmTx_queue_menb_ue"<<m_rnti<<"_bearer"<< (uint16_t)(drbid) << ".txt";

       std::ofstream* stream = new std::ofstream(fileName.str().c_str(), std::ios_base::app);
       rlc->SetStreamForQueueStatistics(stream);//sjkang0827

    if (GetIsMc())
    	m_rlcMap.find(drbid)->second->m_rlc = rlc;
    else if (GetIsMc_2())
    	m_rlcMap.find(drbid)->second->m_rlc_2 = rlc;
...
for ( std::map <uint8_t, Ptr<RlcBearerInfo> >::iterator rlcIt = m_rlcMap.begin ();
        ...
        ++rlcIt)
  {
	// buffer forwarding or copy event occurs when RLC buffer is empty/
	//if (rlcIt->second->m_rlc->GetObject<LteRlcAm>()->GetTxBuffer().size() != 0 or
		//		rlcIt->second->m_rlc_2->GetObject<LteRlcAm>()->GetTxBuffer().size() != 0 )
	{
	if (option == false)
	{
...
// isReceivedmmWaveCell = false;
 // LL = false; LN=false; NN=false, NL=false;
}
...
LteEnbRrc::GetTypeId (void)
{
  ...
  static TypeId tid = TypeId ("ns3::LteEnbRrc")
    ...
    .AddAttribute ("OutageThreshold",
                   ...
                   DoubleValue (-5.0), /// original value is -5  sjkang1117
  ...
  if(alreadyAssociatedImsi && m_imsiUsingLte[imsi])
  {
    ...
    if(!m_interRatHoMode)
    {
      ...
      // it is on LTE, but now the last used MmWave cell is not in outage
      {
        ...
        ueMan->SendRrcConnectionSwitch_2(useMmWaveConnection);
      }
      else if (m_lastMmWaveCell[imsi] != maxSinrCellId && !onHandoverImsi)
...
//////////////////////////////////////////////////////////////////////
   if(handoverNeeded && m_mmWaveCellSetupCompleted[imsi])
  {
  ...
  //  std::cout << imsi << std::endl;
    ...
    if(handoverEvent != m_imsiHandoverEventsMap.end()) // another event was scheduled, but it was already deleted. Replace the entry
    {
      ...
      handoverEvent->second.scheduledHandoverEvent.Cancel(); //sjkang0828
...
//////////////////////////////////////////////////////////////////////
   if(handoverNeeded && m_mmWaveCellSetupCompleted[imsi])
  {
    ...
    std::cout << "m_imsiHandoverEventMap_2.size " << m_imsiHandoverEventsMap_2.size() << std::endl;
    // when the handover event happens, we need to check that no other procedures are ongoing and in case
    // postpone it!
  }
}
...
LteEnbRrc::ComputeTtt(double sinrDifference)
{
 NS_LOG_FUNCTION(this);
  ...
  {
	  //NS_FATAL_ERROR( "unsupporte HO MODE");
	/*  if(sinrDifference < m_minDiffTttValue)
	      {
	        return m_maxDynTttValue;
	      }
...
{
	std::cout<<imsi << " PerformHandover ------------>  " << index<< std::endl;
  NS_LOG_FUNCTION(this);
 // StartHandover = Simulator::Now().GetMilliSeconds();
  ...
  if (index == 1) //sjkang index indicates the mmWave eNB 1, 2
	...
	LteEnbRrc::HandoverEventInfo handoverInfo;
  if (index == 1 ) { //sjkang0710//73G Handover
	  ...
	  if (To_28[imsi]){
		  EpcX2SapProvider::SendBufferDuplicationMessage message;
		  				...
		  				message.imsi = imsi;
		  				message.cellIDforBufferForwarding = handoverInfo.targetCellId;
		  				m_x2SapProvider->DuplicateRlcBuffer(message);
	  }

  }
  else if (index == 2 ) //sjkang0710 // 28G Handover
  {
	   ...
	   HandoverEventMap::iterator handoverEvent = m_imsiHandoverEventsMap.find(imsi);
	   	  if (handoverEvent != m_imsiHandoverEventsMap.end())
	   		  handoverEvent->second.scheduledHandoverEvent.Cancel();
	   	  if (To_73[imsi]){ // this is 28G gNB handover, but if the data path is set to 73G, then we will forward the RLC buffer to 28G gNB
	   		EpcX2SapProvider::SendBufferDuplicationMessage message;
...
void
LteEnbRrc::ThresholdBasedSecondaryCellHandover_2(std::map<uint64_t, CellSinrMap>::iterator imsiIter, double sinrDifference, uint16_t maxSinrCellId, double maxSinrDb)
{
  NS_LOG_FUNCTION(this);
	uint64_t imsi = imsiIter->first;
  bool alreadyAssociatedImsi = false;
  ...
  if(maxSinrCellId == m_secondMmWaveCellForImsiMap[imsi] && !m_imsiUsingLte[imsi])
  {
    if (alreadyAssociatedImsi && !onHandoverImsi && m_lastMmWaveCell[imsi] != maxSinrCellId && sinrDifference > m_sinrThresholdDifference) // not on LTE, handover between MmWave cells
    // this may happen when channel changes while there is an handover
    {
    ...
    //        handoverEvent->second.scheduledHandoverEvent.Cancel();
      ...
      m_mmWaveCellSetupCompleted[imsi] = false; // TODO check this bool
      m_secondMmWaveCellForImsiMap[imsi] = maxSinrCellId;
      NS_LOG_INFO("For imsi " << imsi << " the best cell is " << m_bestMmWaveCellForImsiMap[imsi] << " with SINR " << maxSinrDb);
    }
    else if(alreadyAssociatedImsi && !onHandoverImsi && m_lastMmWaveCell[imsi] != maxSinrCellId && sinrDifference < m_sinrThresholdDifference)
    {
  ...
  else
  {
    ...
    m_secondMmWaveCellForImsiMap[imsi] = maxSinrCellId;
    NS_LOG_INFO("For imsi " << imsi << " the best cell is " << m_secondMmWaveCellForImsiMap[imsi] << " with SINR " << maxSinrDb);
  }
}
//std::ofstream LN_log ("log_LOS_NLOS.txt");
void
LteEnbRrc::HandoverLteToMmWave(uint16_t imsi, uint16_t maxSinrCellId, uint16_t enbType){
bool onHandoverImsi = !m_mmWaveCellSetupCompleted[imsi];
uint16_t lastCellID, lastCellID_2;
if (enbType == 1)
{	lastCellID = m_lastMmWaveCell[imsi];
	lastCellID_2 = m_lastMmWaveCell_2[imsi];
}
else if (enbType ==2){
	lastCellID_2 = m_lastMmWaveCell[imsi];
	lastCellID = m_lastMmWaveCell_2[imsi];
}
if (m_imsiUsingLte[imsi]){
	if(!m_interRatHoMode)
	    {
	      ...
	      else if(lastCellID == maxSinrCellId && !onHandoverImsi)
	      // it is on LTE, but now the last used MmWave cell is not in outage
	      {
	        // switch back to MmWave
	        NS_LOG_INFO("----- on LTE, switch to lastMmWaveCell " << m_lastMmWaveCell[imsi] << " at time " << Simulator::Now().GetSeconds());
	 	      ...
	 	      // it is on LTE, but now a MmWave cell different from the last used is not in outage, so we need to handover
	      {
	        ...
	        params.imsi = imsi;
	        params.targetCellId = maxSinrCellId;
	        params.oldCellId = lastCellID;
	        params.secondMmWaveCellID = lastCellID_2; //sjkang0718
	        std::cout<< "LteEnbRrc::will send handover request to Enb //sjknag1109 " <<std::endl;
	         NS_LOG_INFO(params.imsi);
	        m_x2SapProvider->SendMcHandoverRequest(params); //sjkang
...
double
LteEnbRrc::GetVariance(std::vector<double> SinrSet, double mean){
	double sqrSum = 0;
	uint16_t size = SinrSet.size();
	while (SinrSet.size() !=0 ){
		sqrSum += std::pow((mean-*(SinrSet.begin())),2);
	   SinrSet.erase(SinrSet.begin());
	}
	return sqrSum/size;
}
double
LteEnbRrc::GetMean(std::vector<double> SinrSet){
	uint16_t size = SinrSet.size();
	double sum = 0;
	while (SinrSet.size()!=0 ){
		sum += *(SinrSet.begin());
...
std::ofstream TimeDuplication("TimeDuplication.txt");
void 
...

{
  ...
  if(m_imsiCellSinrMap.size() > 0) // there are some entries
  {
    for(std::map<uint64_t, CellSinrMap>::iterator imsiIter = m_imsiCellSinrMap.begin(); imsiIter != m_imsiCellSinrMap.end(); ++imsiIter)
    {
     ...
     long double currentSinr_2 =0;
      ...
      double * CellIdToSinrVar = new double[20];
      double * CellIdToSinrMean = new double[20];
      for(CellSinrMap::iterator cellIter = imsiIter->second.begin(); cellIter != imsiIter->second.end(); ++cellIter)
      {
        ...
        if (imsi == 1){
        if (SinrSample[cellIter->first].size() < 100)
        	SinrSample[cellIter->first].push_back(10*std::log10(cellIter->second));
        else{
        	SinrSample[cellIter->first].erase(SinrSample[cellIter->first].begin());
        	SinrSample[cellIter->first].push_back(10*std::log10(cellIter->second));
        	double mean = GetMean(SinrSample[cellIter->first]);
        	double var = GetVariance(SinrSample[cellIter->first], mean);
        	CellIdToSinrVar[cellIter->first] =std::sqrt (var);
        	CellIdToSinrMean[cellIter->first] = mean;
        		}
        }
 ...
 //     std::cout << maxSinrCellId<<"\t"<<maxSinrDb << "\t"<<secondMaxSinrCellId<<"\t"<<secondMaxSinrDb <<std::endl;
      ...
      if (((maxSinrDb < m_outageThreshold && secondMaxSinrDb <m_outageThreshold) || (m_imsiUsingLte[imsi] &&
    		  maxSinrDb < m_outageThreshold + 2 && secondMaxSinrDb<m_outageThreshold+2)) && alreadyAssociatedImsi) // no MmWaveCell can serve this UE
      { //sjkang_handover event occurs , it is the case of outage event of mmWave
         ...
         if(m_imsiUsingLte[imsi] == false)
        {
          ...
          bool useMmWaveConnection = false; 
          m_imsiUsingLte[imsi] = !useMmWaveConnection;
          if (m_mmWaveCellSetupCompleted[imsi])
          ueMan->SendRrcConnectionSwitch(useMmWaveConnection);
         ...
         if(handoverEvent_2 != m_imsiHandoverEventsMap_2.end())
                   {
        ...
        if(m_handoverMode == THRESHOLD)
        {
           	if (GetRntiFromImsi(imsi) && !m_updatestop[imsi]){ //sjkang
                		if( GetUeManager(GetRntiFromImsi(imsi))->GetState() == UeManager::CONNECTED_NORMALLY or
                			...
                			m_bestMmWaveCellForImsiMap[imsi] = maxSinrCellId;
                			m_secondMmWaveCellForImsiMap[imsi]=secondMaxSinrCellId;
                		//	uint16_t CellId_28 = m_lastMmWaveCell_2[imsi];
                		//	uint16_t CellId_73 = m_lastMmWaveCell[imsi];
        uint16_t threshold = 5;
      if (currentSinrDb < threshold && currentSinrDb_2 < threshold && !m_imsiUsingLte[imsi]){
    	  // maxSinrDb: 73, secondMaxSinrDb 28
    	  if (maxSinrDb > secondMaxSinrDb)
    	  	  {
    		// 73G eNB Handover this is main link
    		  TttBasedHandover_mmWave1(imsiIter,sinrDifference, maxSinrCellId, maxSinrDb);
    		  To_73[imsi] = true; To_28[imsi] =false; //it is just for one UE case
    	  	  }
    	  else if (maxSinrDb <= secondMaxSinrDb)
    	  	  {
    		  //28G eNB Handover
    		  	  To_73[imsi] = false; To_28[imsi]= true;
    			  TttBasedHandover_mmWave2(imsiIter, sinrDifference_2, secondMaxSinrCellId, secondMaxSinrDb);
     	  	  }
       //if (!NN[imsi] or !LL[imsi])
    	//  GetUeManager(GetRntiFromImsi(imsi))->changePathAtPdcp(m_lastMmWaveCell[imsi], m_lastMmWaveCell_2[imsi]);// To 73G
    	 // GetUeManager(GetRntiFromImsi(imsi))->SetDuplicationMode(true);

    	  TimeDuplication <<"Handover"<<"\t"<< Simulator::Now().GetSeconds() << "\t" << LL[imsi] << "\t"<< LN[imsi]<<"\t"<<NL[imsi]<<NN[imsi]<< std::endl;
    	  // LN: 73G-> LOS, 28G->NLOS,:: NL: 28G->NLOS, 73G->LOS
    	  NN[imsi] = true; LN[imsi]=false; LL[imsi]=false;NL[imsi]=false;
          	}
          	  // when two eNB are LOS sates and Packet Duplication mode would be disabled
        if (currentSinrDb > threshold && currentSinrDb_2 > threshold && !m_imsiUsingLte[imsi] ){

        	  	GetUeManager(GetRntiFromImsi(imsi))->SetDuplicationMode(false);
        	  	if (currentSinrDb > currentSinrDb_2){
        	  		GetUeManager(GetRntiFromImsi(imsi))->changePathAtPdcp(m_lastMmWaveCell[imsi], m_lastMmWaveCell[imsi]);// To 73G
        	  		To_73[imsi] = true; To_28[imsi] = false;
        	  	}
        	  	else if (currentSinrDb_2 > currentSinrDb){
        			GetUeManager(GetRntiFromImsi(imsi))->changePathAtPdcp(m_lastMmWaveCell_2[imsi], m_lastMmWaveCell_2[imsi]);// To 28G
        			To_28[imsi] = true; To_73[imsi] = false;
        	  		}
        		TimeDuplication <<"BufferForwarding_twoSINR" << "\t"<< Simulator::Now().GetSeconds() << "\t"<<To_28[imsi]<<"\t"<<To_73[imsi]<<"\t" << LL[imsi] << "\t"<< LN[imsi]<<"\t"<<NL[imsi]<<"\t"<<NN[imsi]<< std::endl;
        		LL[imsi] = true;LN[imsi] =false; NN[imsi] =false;NL[imsi] = false;
          	  	}

         if (currentSinrDb >threshold && currentSinrDb_2 <threshold && !m_imsiUsingLte[imsi]){
        	 // status is 28G-> NLOS and 73G-> LOS, and data transmission link is 28G

        	 if (m_mmWaveCellSetupCompleted[imsi] and  (NL[imsi] or LL[imsi] or NN[imsi]) and To_28[imsi])
        	{
				EpcX2SapProvider::SendBufferDuplicationMessage message;
				message.targetCellId = m_lastMmWaveCell_2[imsi]; // target is 28G
				message.imsi = imsi;
				message.cellIDforBufferForwarding = m_lastMmWaveCell[imsi];
				TimeDuplication <<"BufferForwarding_73G" << "\t"<< Simulator::Now().GetSeconds() << "\t" << LL[imsi] << "\t"<< LN[imsi]<<"\t"<<NL[imsi]<<NN[imsi]<< std::endl;
				// transmit buffered data at 28G BS to 73G BS
				m_x2SapProvider->DuplicateRlcBuffer(message);
        		}
    	  	  	  GetUeManager(GetRntiFromImsi(imsi))->SetDuplicationMode(false);
    	  	  	  // path switching to 73G BS
    	  	  	  GetUeManager(GetRntiFromImsi(imsi))->changePathAtPdcp(m_lastMmWaveCell[imsi], m_lastMmWaveCell[imsi]); // To 73G
    	  	  	  To_73[imsi] = true; To_28[imsi] = false;
    	  	  	  LL[imsi] = false; LN[imsi]=true;NN[imsi]= false;NL[imsi]=false;
          }

         if (currentSinrDb <= threshold && currentSinrDb_2 >= threshold && !m_imsiUsingLte[imsi] ){ //currentSinrDb_2 is 28G, currentSinrDb is 73G8*/
        	 // status is 73G-> NLOS and 28G-> LOS, and data transmission link is 73G
        	 if (m_mmWaveCellSetupCompleted[imsi] and  (LL[imsi] or  LN[imsi] or NN[imsi]) and To_73[imsi]){
                  				EpcX2SapProvider::SendBufferDuplicationMessage message;
                  				message.targetCellId = m_lastMmWaveCell[imsi];
                  				message.imsi = imsi;
                  				message.cellIDforBufferForwarding = m_lastMmWaveCell_2[imsi];
                  				m_x2SapProvider->DuplicateRlcBuffer(message);
                  	TimeDuplication <<"BufferForwarding_28G" << "\t"<< Simulator::Now().GetSeconds() << "\t" << LL[imsi] << "\t"<< LN[imsi]<<"\t"<<NL[imsi]<<NN[imsi]<< std::endl;


        				}
                  // change the path to LOS eNB; 28G
                  GetUeManager(GetRntiFromImsi(imsi))->changePathAtPdcp(m_lastMmWaveCell_2[imsi], m_lastMmWaveCell_2[imsi]);
                  To_73[imsi] = false; To_28[imsi] = true;
                  // Turn off the duplication mode in case of it turns on
                  GetUeManager(GetRntiFromImsi(imsi))->SetDuplicationMode(false);
                	LL[imsi]=false;NL[imsi]=true; NN[imsi]=false;LN[imsi]= false;
                  		}
                	}

/*       if (To_28[imsi] && CellIdToSinrVar[m_lastMmWaveCell_2[imsi]] > 2){
        	GetUeManager(GetRntiFromImsi(imsi))->changePathAtPdcp(m_lastMmWaveCell[imsi], m_lastMmWaveCell_2[imsi]);// To 28G
        	  GetUeManager(GetRntiFromImsi(imsi))->SetDuplicationMode(true);
        }
        else if (To_73[imsi] && CellIdToSinrVar[m_lastMmWaveCell[imsi]] > 2){
         	  	GetUeManager(GetRntiFromImsi(imsi))->changePathAtPdcp(m_lastMmWaveCell[imsi], m_lastMmWaveCell_2[imsi]);// To 28G
         	  	GetUeManager(GetRntiFromImsi(imsi))->SetDuplicationMode(true);
        }
        else {
       ...
       /* else{
        		if (To_28[imsi]){
        		  	...
        		  	GetUeManager(GetRntiFromImsi(imsi))->changePathAtPdcp(m_lastMmWaveCell[imsi], m_lastMmWaveCell[imsi]);// To 28G
        		}
        	  GetUeManager(GetRntiFromImsi(imsi))->SetDuplicationMode(false);
        }*/
      /*  if (imsi == 1)
        	uevariance << Simulator::Now().GetSeconds() <<"\t" <<CellIdToSinrVar[m_lastMmWaveCell_2[imsi]]
				<<"\t"<<CellIdToSinrMean[m_lastMmWaveCell_2[imsi]]<<"\t"<< CellIdToSinrVar[m_lastMmWaveCell[imsi]]<<
						"\t"<<CellIdToSinrMean[m_lastMmWaveCell[imsi]]<< std::endl;*/

        delete[] CellIdToSinrVar;
        delete[] CellIdToSinrMean;

          if (maxSinrDb > secondMaxSinr && m_imsiUsingLte[imsi])
               HandoverLteToMmWave(imsi, maxSinrCellId,1);
          else if (secondMaxSinrDb > maxSinrDb && m_imsiUsingLte[imsi])
               HandoverLteToMmWave(imsi, secondMaxSinrCellId,2);
...
/// in the case of sending Ack to Lte eNB , then target eNB will send imsi information , instead of old rnti info //sjkang0417
  ...
  m_x2SapProvider->SendHandoverRequestAck(ackParams); // Target eNB sends Ack message to Source eNB
...
/*uint16_t imsi = GetImsiFromRnti(params.oldEnbUeX2apId);
	EpcX2SapProvider::SendBufferDuplicationMessage message;
	if (m_index == 2 && NN){
        message.targetCellId = m_lastMmWaveCell[imsi];
         message.imsi = imsi;
         message.cellIDforBufferForwarding = m_secondMmWaveCellForImsiMap[imsi];
           //message.option = false; //true means buffer forwarding
         m_x2SapProvider->DuplicateRlcBuffer(message);
	}*/
...
LteEnbRrc::DoRecvAssistantInfo(EpcX2Sap::AssistantInformationForSplitting params){ //sjkang1114
 ...
 ////CellID->Rnti->Imsi
  // RntiAndImsi rntiAndImsi = GetImsiFromCellId_Rnti[params.sourceCellId]; //sjkang
   //params.rnti is imsi
	//GetUeManager(params.rnti)->RecvAssistantInfo(params); //sjkang1115
}
void
...
/*
void
LteEnbRrc::DoSendDlCQI(Ptr<MmWaveControlMessage> msg){
//	std::cout << "received " <<std::endl;
  Ptr<MmWaveDlCqiMessage> cqi = DynamicCast<MmWaveDlCqiMessage>(msg);
  DlCqiInfo dlcqi = cqi->GetDlCqi();
