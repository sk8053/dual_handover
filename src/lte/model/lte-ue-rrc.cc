...
namespace ns3 {
	...
	void
		LteUeRrc::DoDispose ()
		{
			...
			std::cout<< "UE will destroy drb in LTE-UE-RRC" <<std::endl;
	...
	void
		LteUeRrc::SetMmWaveUeCphySapProvider_2 (LteUeCphySapProvider * s)
		{
			NS_LOG_FUNCTION (this << s);
	...
	void
		LteUeRrc::SetMmWaveUeCmacSapProvider_2 (LteUeCmacSapProvider * s)
		{
			NS_LOG_FUNCTION (this << s);
	...
	void
		LteUeRrc::SetMmWaveMacSapProvider_2 (LteMacSapProvider * s) //sjkang
		{
			NS_LOG_FUNCTION (this << s);
			//m_macSapProvider_2 = s;
			//m_lteMacSapProvider_2 = s;
		...
		LteUeRrc::SwitchLowerLayerProviders (uint16_t cellId)
		{
			...
			if(m_isMmWaveCellMap.find(cellId) != m_isMmWaveCellMap.end()) ///if mmWave cell is true
			{
				if(m_isMmWaveCellMap.find(cellId)->second)
				{
					...
					std::cout<<"LteUeRrc : Switch Lower Layer Provider"<<this<<" "<< sw<< std::endl;
					if(sw==0){ //sjkang
	...
	void
		LteUeRrc::DoSendData (Ptr<Packet> packet, uint8_t bid)
		{
			...
			NS_LOG_INFO("send data in LteUeRrc to Pdcp Layer");
	...
	void
		LteUeRrc::DoNotifySecondaryCellConnected(uint16_t mmWaveRnti, uint16_t mmWaveCellId)
		{
			...
			NS_LOG_FUNCTION(this);
			m_rrcSapUser->SendNotifySecondaryCellConnected(mmWaveRnti, mmWaveCellId); ///will go to mmWave-lte-rrc-protocol-real
		}
	void
		LteUeRrc::DoNotifySecondaryCellConnected(uint16_t mmWaveRnti_73G,uint16_t mmWaveRnti_28G, uint16_t mmWaveCellId_73G, uint16_t mmWaveCellId_28G)
		{
			NS_LOG_FUNCTION (this);
			...
			NS_LOG_FUNCTION(this);
			m_rrcSapUser->SendNotifySecondaryCellConnected(mmWaveRnti_28G, mmWaveCellId_28G); ///will go to mmWave-lte-rrc-protocol-real
		}
	void
				...
				case IDLE_RANDOM_ACCESS:
					{
						...
						msg.isMc_2=m_isThirdRrc;// 73G connection // 73G firstly connected
	...
	void 
		LteUeRrc::DoRecvSystemInformation (LteRrcSap::SystemInformation msg) //sjkang
		{
	...
	void 
		LteUeRrc::DoRecvRrcConnectToMmWave (uint16_t mmWaveCellId, uint16_t mmWaveCellId_2) //sjkang
		{
						...
						if (msg.haveMobilityControlInfo)
						{
							...
							for (iter = m_rlcMap.begin(); iter!=m_rlcMap.end(); iter++){ ///sjkang0709
								if (msg.HandoverCase == 2) {
									Ptr<LteRlc > rlc = iter->second->m_rlc_2->GetObject<LteRlc>();
									iter->second->Cleanup();
									iter->second->m_rlc_2 = rlc;
								}else if (msg.HandoverCase == 1){
									Ptr<LteRlc > rlc = iter->second->m_rlc->GetObject<LteRlc>();
									iter->second->Cleanup();
				...
				if(m_drbMap.find(*iter) != m_drbMap.end())
				{
					...
					if(pdcp != 0)
					{
						...
						if(msg.useMmWaveConnection) // I was on LTE, now I switch to mmWave
						{
							...
							fileName_0<<"rlc_Tput_menb_ue"<<m_imsi<<"_bearer_"<< (uint16_t)(m_drbMap.find(*iter)->second->m_drbIdentity) << ".txt";
							std::ofstream* streamPathThroughput_0 = new std::ofstream(fileName_0.str().c_str(), std::ios_base::app);
						...
						else // I was on mmWave, and I switch to LTE
						{
							...
							Ptr<LteRlc> rlc_2 = rlcObjectFactory.Create ()->GetObject<LteRlc> ();
							rlc_2->SetLteMacSapProvider (m_mmWaveMacSapProvider_2);
							...
							fileName << "rlc_Tput_senb2_ue"<<m_imsi<<"_bearer_"<< (uint16_t)(m_drbMap.find(*iter)->second->m_drbIdentity) << ".txt";
							std::ofstream* streamPathThroughput_1 = new std::ofstream(fileName.str().c_str(), std::ios_base::app);
							rlc->CalculatePathThroughput(streamPathThroughput_1);

							std::ostringstream fileName_2;
							fileName_2 <<"rlc_Tput_senb1_ue"<<m_imsi<<"_bearer_"<< (uint16_t)(m_drbMap.find(*iter)->second->m_drbIdentity) << ".txt";
							std::ofstream* streamPathThroughput_2 = new std::ofstream(fileName_2.str().c_str(), std::ios_base::app);
							...
							m_mmWaveCmacSapProvider->AddLc (m_rlcMap.find(*iter)->second->logicalChannelIdentity, 
									lcConfig,
									rlc->GetLteMacSapUser ());
							m_mmWaveCmacSapProvider_2->AddLc (m_rlcMap.find(*iter)->second->logicalChannelIdentity,
									lcConfig,
									rlc_2->GetLteMacSapUser ());

							if (rlcTypeId != LteRlcSm::GetTypeId ())
							{
								pdcp->SetMmWaveRnti (m_mmWaveRnti_73, m_mmWaveCellId_28G);
								pdcp->SetMmWaveRlcSapProvider (rlc->GetLteRlcSapProvider ());
								pdcp ->SetMmWaveRlcSapProvider_2(rlc_2->GetLteRlcSapProvider());
								rlc->SetLteRlcSapUser (pdcp->GetLteRlcSapUser ());
								rlc_2->SetLteRlcSapUser(pdcp->GetLteRlcSapUser ());
	...
	void //LTE eNB RRC will run this function
		...
		{
			NS_ASSERT_MSG(!m_isSecondaryRRC or !m_isThirdRrc, "Trying to modify RLCs in the mmWave RRC (they are instances in the LTE RRC)");
				...
				if(dtamIt->is_mc == true || dtamIt->is_mc_2) // we need to modify the RLC for MmWave communications
				{
					...
					if(pdcp !=0)
					{
						...
						if (dtamIt->is_mc){
							...
							rlc->SetLteMacSapProvider (m_mmWaveMacSapProvider_2);
						...
						//	   lcConfig, rlc_2->GetLteMacSapUser());
						if (dtamIt->is_mc)
						...
						if (rlcTypeId != LteRlcSm::GetTypeId ())
						{
							pdcp->SetMmWaveRnti (m_mmWaveRnti_73, m_mmWaveCellId_28G);
							//	rlc_2->SetLteRlcSapUser(pdcp->GetLteRlcSapUser());
							//	pdcp->SetMmWaveRlcSapProvider(rlc_2->GetLteRlcSapProvider());
							if (dtamIt->is_mc)
								pdcp->SetMmWaveRlcSapProvider (rlc->GetLteRlcSapProvider ());
							else if (dtamIt->is_mc_2)
								pdcp->SetMmWaveRlcSapProvider_2(rlc->GetLteRlcSapProvider());

							rlc->SetLteRlcSapUser (pdcp->GetLteRlcSapUser ());
							//   rlc_p->SetLteRlcSapUser(pdcp->GetLteRlcSapUser());
						...
						if (dtamIt->is_mc){
							...
							rlcInfo->m_rlc = rlc;
							//	  pdcp->SetMmWaveRlcSapProvider_2(rlc_p->GetLteRlcSapProvider());
						}
						else if (dtamIt->is_mc_2){
							rlcInfo->m_rlc = rlc_p;
							rlcInfo->m_rlc_2 = rlc;
							//    pdcp->SetMmWaveRlcSapProvider(rlc_p->GetLteRlcSapProvider());
						}
						...
						NS_LOG_INFO(this << "  LteUeRrc Secondary Cell Handover, created new rlc " << m_imsi <<m_mmWaveCellId_28G<<"\t"
					...
					++dtamIt)
			{
				...
				if (drbMapIt == m_drbMap.end ())
				{
					...
					m_cmacSapProvider->AddLc (dtamIt->logicalChannelIdentity,
							...
							rlc->GetLteMacSapUser ());
					std::ostringstream fileName_0;
					//  fileName_0<<"UE-"<<m_imsi<<"-LTE-" << "Bearer-"<< (uint16_t)(dtamIt->drbIdentity) << "-RLC-Throughput.txt";
				...
				else
				{
					...
					Ptr<LteDataRadioBearerInfo> drbInfo = drbMapIt->second;
					NS_LOG_INFO ("Is MC " << dtamIt->is_mc);
					if(dtamIt->is_mc == true or dtamIt->is_mc_2 ==true) // we need to setup the RLC for MmWave communications  //sjkang1109 i gues lte drb already setup
					{
						Ptr<McUePdcp> pdcp = DynamicCast<McUePdcp> (drbInfo->m_pdcp);
						if(pdcp !=0)
						{
							...
							//  Simulator::Schedule(MilliSeconds(100),&LteRlcUm::)
							std::ostringstream fileName_2;
							fileName_2 <<"rlc_Tput_senb1_ue"<<m_imsi<<"_bearer_"<< (uint16_t)(dtamIt->drbIdentity) << ".txt";
							...
							////sjkang1110
							Ptr<LteRlc> rlc_2 = rlcObjectFactory.Create ()->GetObject<LteRlc> ();
							rlc_2->SetLteMacSapProvider (m_mmWaveMacSapProvider_2); //sjkang1110
							...
							m_mmWaveCmacSapProvider_2->AddLc (dtamIt->logicalChannelIdentity,
									lcConfig,
									rlc_2->GetLteMacSapUser ()); //sjkang1109

							if (rlcTypeId != LteRlcSm::GetTypeId ())
							{
								pdcp->SetMmWaveRnti (m_mmWaveRnti_73, m_mmWaveCellId_28G);
								pdcp->SetMmWaveRlcSapProvider (rlc->GetLteRlcSapProvider ());
								pdcp->SetMmWaveRlcSapProvider_2 (rlc_2->GetLteRlcSapProvider ());//sjkang1110

								rlc->SetLteRlcSapUser (pdcp->GetLteRlcSapUser ());
								rlc_2->SetLteRlcSapUser (pdcp->GetLteRlcSapUser ()); //sjkang1110
							} 
							rlc->Initialize();
