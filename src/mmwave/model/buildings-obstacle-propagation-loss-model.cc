...
BuildingsObstaclePropagationLossModel::GetLoss (Ptr<MobilityModel> a, Ptr<MobilityModel> b) const
	...
	if (a1->IsOutdoor () && b1->IsOutdoor ())
	{
		...
		//	std::cout << angle << std::endl;
			...
			//	Vector loc1(boundaries.xMin,boundaries.yMin,boundaries.zMin); //sjkang

				Vector loc2(boundaries.xMin,boundaries.yMax,boundaries.zMin);
			//	Vector loc2(boundaries.xMax,boundaries.yMax,boundaries.zMin); //sjkang
			...
			//			&& angles1.phi < M_PI/2 && angles2.phi <M_PI/2 && locationA.x<boundaries.xMax &&
			//			locationA.y<boundaries.yMax ) //sjkang1206
				{
					los = false;
			...
			//			&& locationA.x < boundaries.xMin  ) //sjkang1206
				{
					los = false;
	...
	//	 std::cout<<this <<  a->GetPosition().x <<"\t"<< b->GetPosition().x << "\t"<< nlosSamples<<"\t"<<losSamples << std::endl;
		...
		else
		{
			...
			else if (los && losSamples < g_nlosSamplesTrace && nlosSamples > 0) // I am in NLOS but in the 'raise phase'
			{
				...
				/*	 if((a->GetPosition().x ==200 && b->GetPosition().x == 100) ||(a->GetPosition().x ==200 && b->GetPosition().x == 200))
					 	 std::cout<<Simulator::Now().GetSeconds() <<"\t"<<  a->GetPosition().x <<"\t"<< b->GetPosition().x <<
						 "\t"<< sample_trace << std::endl;*/
...
if (additionalPath){
	...
	if (t > 2 && t < 3)
	{
		//m_beamforming_2->UpdateMatrices(false);
		loss = loss + 50;
	}
	else if (t >4  && t < 4.5)
	{
...
void
BuildingsObstaclePropagationLossModel::SetBeamforming_2 (Ptr<MmWaveBeamforming> beamforming)//sjkang
{
	m_beamforming_2 = beamforming;
}
