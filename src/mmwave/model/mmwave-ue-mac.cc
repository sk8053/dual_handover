...
MmWaveUeMac::DoReceivePhyPdu (Ptr<Packet> p)
{
  ...
  NS_LOG_INFO("ReceivePdu for rnti " << tag.GetRnti());
	if (tag.GetRnti () == m_rnti) // packet is for the current user
	{
		...
		NS_LOG_INFO("MmWaveUeMac receive PHY pdu with " << macSubheaders.size() << " pdu");
		uint32_t currPos = 0;
...
{
	NS_LOG_INFO (this << " lcId" << (uint32_t) lcId);
...
MmWaveUeMac::DoRemoveLc (uint8_t lcId)
{
  ...
  //std::cout<<"MmWave UE Mac will Remove LC in " << m_rnti << " lcid " << (uint32_t)lcId << std::endl;
