	...
	for (BuildingList::Iterator bit = BuildingList::Begin (); bit != BuildingList::End (); ++bit)
	{
		...
		if (angle >=0 && angle < M_PI/2 )
		{
			...
			Angles angles2 (loc2,locationA);
			 ...
			 if (angle > angles1.phi && angle < angles2.phi && locationB.x > boundaries.xMin &&
									 locationB.y > boundaries.yMin && locationA.x <boundaries.xMax)
						{
							los = false; // los = false means drop phase
		...
		else if (angle >= -M_PI/2 && angle < 0)
		{
			...
			Angles angles2 (loc2,locationA);
			  ...
			  if (angle > angles1.phi && angle < angles2.phi && locationB.x > boundaries.xMin
					 && locationB.y < boundaries.yMax && locationA.x <boundaries.xMax)
			{
				los = false;
