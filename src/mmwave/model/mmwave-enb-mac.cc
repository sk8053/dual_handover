...
//std::ofstream MacPdu_ ("MacPduBuffer.txt");
void
MmWaveEnbMac::DoTransmitPdu (LteMacSapProvider::TransmitPduParameters params)
{
	...
	//MacPdu_<<Simulator::Now().GetSeconds() <<"\t" << m_macPduMap.size() <<"\t"<<params.pdu->GetSize ()<< std::endl;
		...
		if (slotAllocInfo.m_slotType != SlotAllocInfo::CTRL && slotAllocInfo.m_tddMode == SlotAllocInfo::DL)
		{
			...
			else
			{
				...
				if (dciElem.m_ndi == 1)
				{
					...
					//std::cout<<"Enb will transmit Mac pdu  " << this << "\t" <<Simulator::Now().GetSeconds()<<"\t"
					//	<<pduMapIt->second.m_pdu->GetSize() <<"\t" << pduMapIt->second.m_pdu->GetSize() <<std::endl;

					m_phySapProvider->SendMacPdu (pduMapIt->second.m_pdu); //sjkang
