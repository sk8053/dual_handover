...
MmWaveBeamforming::Initial(NetDeviceContainer ueDevices, NetDeviceContainer enbDevices)
{
	...
	for (NetDeviceContainer::Iterator i = ueDevices.Begin(); i != ueDevices.End(); i++)
	{
		for (NetDeviceContainer::Iterator j = enbDevices.Begin(); j != enbDevices.End(); j++)
		{
			if(m_update)
			{
				//SetChannelMatrix (*i,enbDevices.Get(0), enbDevices.Get(1)); // this call is independent on the subclass of NetDevice
				//SetChannelMatrix (*i,enbDevices.Get(1), enbDevices.Get(0)); // this call is independent on the subclass of NetDevice

				//SetBeamformingVector(*i,enbDevices.Get(0), enbDevices.Get(1)); // possible beamforming vector for all the UE eNB pairs

				//SetBeamformingVector(*i,enbDevices.Get(1), enbDevices.Get(0)); // possible beamforming vector for all the UE eNB pairs
...
//void
//MmWaveBeamforming::SetChannelMatrix (Ptr<NetDevice> ueDevice, Ptr<NetDevice> enbDevice_2 , Ptr<NetDevice> enbDevice)
void
...

{
	...
	//key_t key_2 = std::make_pair(ueDevice,enbDevice_2);
	int randomInstance = m_uniformRV->GetValue (0, g_numInstance-1);
//	int randomInstance_2 = m_uniformRV->GetValue (0, g_numInstance-1);
	//NS_LOG_UNCOND (Simulator::Now().GetSeconds() <<" ************* UPDATING CHANNEL MATRIX (instance " << randomInstance << ") *************");
	...
	//Ptr<BeamformingParams> bfParams_2 = Create<BeamformingParams> ();
	//	bfParams_2->m_enbW = g_enbAntennaInstance.at (randomInstance_2);
	//	bfParams_2->m_ueW = g_ueAntennaInstance.at (randomInstance_2);
	//	bfParams_2->m_channelMatrix.m_enbSpatialMatrix = g_enbSpatialInstance.at (randomInstance_2);
	//	bfParams_2->m_channelMatrix.m_ueSpatialMatrix = g_ueSpatialInstance.at (randomInstance_2);
	//	bfParams_2->m_channelMatrix.m_powerFraction = g_smallScaleFadingInstance.at (randomInstance_2);
	//	bfParams_2->m_beam = GetLongTermFading (bfParams_2);

	std::map< key_t, Ptr<BeamformingParams> >::iterator iter = m_channelMatrixMap.find(key);
	//std::map< key_t, Ptr<BeamformingParams> >::iterator iter_2 = m_channelMatrixMap.find(key_2);

	if (iter != m_channelMatrixMap.end () )
	{
		m_channelMatrixMap.erase (iter);
	}
	//if (iter_2 != m_channelMatrixMap.end () )
	//	{
	//		m_channelMatrixMap.erase (iter_2);
	//	}
	m_channelMatrixMap.insert(std::make_pair(key,bfParams));
	//m_channelMatrixMap.insert(std::make_pair(key_2,bfParams_2)); //sjkang1117
...
//void
//MmWaveBeamforming::SetBeamformingVector (Ptr<NetDevice> ueDevice,  Ptr<NetDevice> enbDevice_2, Ptr<NetDevice> enbDevice) //sjk  ang1010
void
MmWaveBeamforming::SetBeamformingVector (Ptr<NetDevice> ueDevice,  Ptr<NetDevice> enbDevice) //sjk  ang1010

{
	...
	//std::map< key_t, Ptr<BeamformingParams> >::iterator it_2 = m_channelMatrixMap.find(key_2);

	NS_ASSERT_MSG (it != m_channelMatrixMap.end (), "could not find");
	Ptr<BeamformingParams> bfParams = it->second;
	//Ptr<BeamformingParams> bfParams_2 = it_2->second;
...
//if(isAdditionalMmWavePhy && enbDevice->GetObject<MmWaveEnbNetDevice>()->isAdditionalEnb ) //sjkang1116
	if(isAdditionalMmWavePhy ) //sjkang1116

{
	antennaPair antennaArrays_2 = GetUeEnbAntennaPair_2(ueDevice, enbDevice); //sjkang
					Ptr<AntennaArrayModel> enbAntennaArray_2 = antennaArrays_2.second; //sjkang
					Ptr<AntennaArrayModel> ueAntennaArray_2 = antennaArrays_2.first;//sjkang

	ueAntennaArray_2->SetBeamformingVector (bfParams->m_ueW, enbDevice); //sjkang//
	enbAntennaArray_2->SetBeamformingVector (bfParams->m_enbW, ueDevice);
}//sjkang
	else   //sjkang1116
	{
		antennaPair antennaArrays = GetUeEnbAntennaPair(ueDevice, enbDevice); //sjkang
						Ptr<AntennaArrayModel> enbAntennaArray = antennaArrays.second; //sjkang
						Ptr<AntennaArrayModel> ueAntennaArray = antennaArrays.first;//sjkang
...
MmWaveBeamforming::DoCalcRxPowerSpectralDensity (Ptr<const SpectrumValue> txPsd,
												Ptr<const MobilityModel> a,
												Ptr<const MobilityModel> b) const /// a is Ue, b is Enb
{
	bool downlink;
	Ptr<NetDevice> enbDevice, ueDevice, enbDevice_2; //sjkang1117
//	std::cout<< " DoCalcRxPowerSpectralDensity  "  << std::endl;
	...
	//Ptr<NetDevice> txDevice_2 = a->GetObject<Node> ()->GetDevice (0);
	//Ptr<NetDevice> rxDevice_2 = b->GetObject<Node> ()->GetDevice (1);

	Ptr<SpectrumValue> rxPsd = Copy (txPsd);
	key_t dlkey = std::make_pair(rxDevice,txDevice);
	key_t ulkey = std::make_pair(txDevice,rxDevice);

	//key_t dlkey_2 = std::make_pair(rxDevice_2,txDevice_2);
	//key_t ulkey_2 = std::make_pair(txDevice_2,rxDevice_2);
...
//	std::map< key_t, Ptr<BeamformingParams> >::iterator it_2;
//	if (m_channelMatrixMap.find(dlkey_2) != m_channelMatrixMap.end ())
//		{
			// this is downlink case
	//		downlink = true;
	//		enbDevice_2 = txDevice_2;
	//		ueDevice = rxDevice_2;
		//	it_2 = m_channelMatrixMap.find(dlkey_2);
		//}
		//else if (m_channelMatrixMap.find(ulkey_2) != m_channelMatrixMap.end ())
	//	{
			// this is uplink case
	//		downlink = false;
	//		ueDevice = txDevice_2;
	//		enbDevice_2 = rxDevice_2;
	//		it_2 = m_channelMatrixMap.find(ulkey_2);
	//	}
	//	else
	//	{
			// enb to enb or ue to ue transmission, set to 0. Do no consider such scenarios.
	...
	antennaPair antennaArrays;// antennaArrays_2;
	if(isAdditionalMmWavePhy ) //sjkang
 	 antennaArrays = GetUeEnbAntennaPair_2(ueDevice, enbDevice);
	else
	...
	//Ptr<AntennaArrayModel> enbAntennaArray_2 = antennaArrays_2.second;
	//	Ptr<AntennaArrayModel> ueAntennaArray_2 = antennaArrays_2.first;
...
//		return rxPsd;//zml do not apply fading to omi
		...
		//	vec.push_back(std::complex<double> (1,0));
		//}
		//(*bfParams_2->m_beam) = vec;
	//}
	//else
//	{
	//	complexVector_t ueW_2 = ueAntennaArray_2->GetBeamformingVector();
	//	complexVector_t enbW_2 = enbAntennaArray_2->GetBeamformingVector();

		//if (!ueW_2.empty() && !enbW_2.empty())
		//{
		//	bfParams_2->m_ueW = ueW_2;
		//	bfParams_2->m_enbW = enbW_2;
		//	bfParams_2->m_beam = GetLongTermFading (bfParams_2);
		//}
		//else if(ueW_2.empty())
		//{
...
//std::cout << "relative speed ---->  "<< relativeSpeed << std::endl;
...
//		std::cout << std::endl;
	...
	else if (ueMc != 0)
	{
		...
		uePhy = ueMc ->GetMmWavePhy_2();
	}
	
	if (downlink)
	{
		NS_LOG_INFO ("****** DL BF gain (RNTI " << uePhy->GetRnti() << ") == " << Sum (bfGain)/nbands << " RX PSD " << Sum(*rxPsd)/nbands <<"\t"<< nbands); // print avg bf gain
	}
	else
	{
...
antennaPair
MmWaveBeamforming::GetUeEnbAntennaPair_2(Ptr<NetDevice> ueDevice, Ptr<NetDevice> enbDevice) const
{
	NS_LOG_FUNCTION(this);
	Ptr<MmWaveEnbNetDevice> EnbDev =
				DynamicCast<MmWaveEnbNetDevice> (enbDevice);
