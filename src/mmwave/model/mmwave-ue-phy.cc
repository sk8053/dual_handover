...
MmWaveUePhy::GetTypeId (void)
{
	static TypeId tid = TypeId ("ns3::MmWaveUePhy")
		...
		.AddAttribute ("OutageThreshold",
                   ...
                   DoubleValue (-5.0), // original value is -5
...
NS_LOG_FUNCTION(this);
	...
	{
		if(enbNetDevice->isEnbTypeForDc) //sjkang
		DynamicCast<McUeNetDevice>(m_netDevice)->SetMmWaveTargetEnb_2(enbNetDevice); //sjkang
		else
		DynamicCast<McUeNetDevice>(m_netDevice)->SetMmWaveTargetEnb(enbNetDevice);//sjkang

	}
	NS_LOG_UNCOND("UE register to enb " << enbNetDevice->GetCellId());
		// call antennaarrya to change the bf vector
	//std::cout<<isAdditionalUePhy << "\t " << cellId << "\t" <<std::endl;
	...
	for (it = msgList.begin (); it != msgList.end (); it++)
	{
		...
		if (msg->GetMessageType() == MmWaveControlMessage::DCI_TDMA)
		{
			...
			SfnSf dciSfn = dciMsg->GetSfnSf ();
				//std::cout<< "MmWaveUePhy receives control message and message type is DCI_TDMA" <<std::endl;
...
//std::ofstream TbSize("TbSize.txt");
void
MmWaveUePhy::StartSlot ()
{
	...
	else if (currSlot.m_dci.m_format == DciInfoElementTdma::DL)  // scheduled DL data slot
	{
		...
		*streamForTbSize <<this << "\t"<<m_imsi<< "\t"<< currSlot.m_dci.m_tbSize<< std::endl; //sjkang0815
	...
	if(m_ulConfigured && (m_rnti > 0) && m_receptionEnabled)
	{
		if (Simulator::Now () > m_wbCqiLast + m_wbCqiPeriod)
		{
			...
			if (msg)
			{
				...
				//std::cout<< m_imsi <<" Send Dl Cqi report to Enb -- > " << this <<"\t" << sinrAvg << std::endl; //sjkang
				*stream<<Simulator::Now().GetSeconds() <<"\t"<<10*log10(sinrAvg)<< std::endl;
...
void
MmWaveUePhy::SetStreamForMeasuringSinr(std::ofstream *str, std::ofstream *stremTbSize){
stream = str;
streamForTbSize = stremTbSize;
}
