...
namespace ns3 {
	...
	void
		...
		//	m_phyMacCommon -> SetAttribute("CenterFreq",DoubleValue(73e9)); //modified by gsoul to connect 28GHZ first. m_phyMacComomon->InstallSingleDevice is connected second 180920
	...
	NetDeviceContainer
		MmWaveHelper::InstallEnbDevice_28GHZ (NodeContainer c)
		{
			...
			for (NodeContainer::Iterator i = c.Begin (); i != c.End (); ++i)
			{
				...
				device = InstallSingleEnbDevice (node); //28GHz //modified by gsoul to connect 28GHz eNB first 180920
	...
	NetDeviceContainer
		MmWaveHelper::InstallEnbDevice_73GHZ (NodeContainer c)
		{
			...
			for (NodeContainer::Iterator i = c.Begin (); i != c.End (); ++i)
			{
				...
				Ptr<NetDevice> device;
				//if(cc==0)
				device = InstallSingleEnbDevice_2 (node);//73GHz //modified by gsoul to connect 73GHz eNB secondly 180920
				device->SetAddress (Mac48Address::Allocate ());
				devices.Add (device);
				//cc++;
			}
	...
	Ptr<NetDevice>
		MmWaveHelper::InstallSingleMcUeDevice(Ptr<Node> n)
		{
			...
			std::ostringstream fileName_1,fileName_2, fileName_TbSize; //sjkang1124
			fileName_1<< "UE-"<<imsi<<"-1-Dl-Sinr.txt"; //1 means 73GHz
			fileName_2 << "UE-"<<imsi<<"-0-Dl-Sinr.txt";  // 0 means 28GHz
			fileName_TbSize<<"UE-"<<imsi << "-TbSize.txt";
			...
			std::ofstream *streamForTbSize = new std::ofstream(fileName_TbSize.str().c_str());
			mmWavePhy_2->SetStreamForMeasuringSinr(stream_1, streamForTbSize);
			mmWavePhy->SetStreamForMeasuringSinr(stream_2, streamForTbSize);
			...
			else if(m_channelModelType == "ns3::MmWaveChannelMatrix")
			{
				...
				m_channelMatrix_2->isAdditionalMmWavePhy=true;
				mmWavePhy_2->AddSpectrumPropagationLossModel(m_channelMatrix_2);
	...
	void
		MmWaveHelper::AttachToClosestEnb (NetDeviceContainer ueDevices, NetDeviceContainer mmWaveEnbDevices,NetDeviceContainer mmWaveEnbDevices_2, NetDeviceContainer lteEnbDevices)
		{
			...
			else if(m_channelModelType == "ns3::MmWaveChannelMatrix")
			{
				...
				m_channelMatrix_2->isAdditionalMmWavePhy = true;
				m_channelMatrix_2->Initial(ueDevices, mmWaveEnbDevices_2);
			}
			else if(m_channelModelType == "ns3::MmWaveChannelRaytracing")
			{
			...
			else if(m_channelModelType == "ns3::MmWave3gppChannel")
			{
				...
				m_3gppChannel_2->Initial(ueDevices,mmWaveEnbDevices_2);//sjkang0724
			}
