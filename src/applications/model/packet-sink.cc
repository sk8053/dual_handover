...
namespace ns3 {
	...
	TypeId 
		PacketSink::GetTypeId (void)
		{
			static TypeId tid = TypeId ("ns3::PacketSink")
				...
				.AddTraceSource("LinkDelay", "this is link delay between apps", MakeTraceSourceAccessor(&PacketSink::m_delay)
	...
	uint16_t
		PacketSink::GetMessageSize(uint16_t messageNumer){
			return m_Messages[messageNumer].size();
		}
	void
		PacketSink::eraseMaps(){
			m_SuccessIndication.clear();
			m_ArrivalTime.clear();
		}
	std::list<Ptr<Socket> >
	...
	/*
	   uint16_t PacketSink::GetReceivedMessageSize() {
	   NS_LOG_FUNCTION(this);
	   return SN.size();
	   }*/
	...
	uint32_t PacketSink::GetReceivedSN() const{
		return m_receivedSN;
	}
	...
	void PacketSink::SetPacketWindowSize(uint16_t size){
		NS_LOG_FUNCTION (this);
		m_lossCounter.SetBitMapSize(size); //sjkang2019
	}
	...
	void PacketSink::HandleRead (Ptr<Socket> socket)
	{
		...
		while ((packet = socket->RecvFrom (from)))
		{
			...
			m_loss(seqTs.GetSeq(),GetLost(),delay);
			m_receivedSN = seqTs.GetSeq(); //sjkang
			m_delay( delay);


			if (messageTransmission){ //sjkang
				uint32_t messageNumber = (m_receivedSN - (m_receivedSN % numberPackets))/numberPackets ;
				m_Messages[messageNumber].insert(std::pair<uint32_t, bool>(m_receivedSN % numberPackets, true));

				for (std::map<uint32_t, OneMessage>::iterator it = m_Messages.begin(); it != m_Messages.end(); it++){
					if ((it->second).size() == numberPackets)
					{
						m_SuccessIndication[it->first] = true;

						for (uint32_t j =0; j < numberPackets;  j++){

							if ((it->second).find(j) == (it->second).end() or (it->second)[j] ==false)
								m_SuccessIndication[it->first] = false;
						}
						m_Messages.erase(it->first);
						m_ArrivalTime[it->first]= Simulator::Now();
