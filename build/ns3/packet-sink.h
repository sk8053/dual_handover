...
public:
  ...
  uint32_t GetReceivedSN(void) const; //sjkang
  uint16_t GetPacketWindowSize() const; //sjkang
  void SetPacketWindowSize(uint16_t); //sjkang
  TracedCallback<uint64_t, uint32_t, double> m_loss;
  TracedCallback<double> m_delay;//sjkang
 std::map<uint32_t, bool> GetSuccess(); //sjkang
 std::map<uint32_t, Time> GetArrivalTime();
 void eraseMaps();
 uint16_t GetMessageSize(uint16_t); //sjkang
 //Time GetArrivalTime();
 //uint16_t GetReceivedMessageSize(); //sjkang
 //void SetClearSnBuffer();

 typedef std::map<uint32_t, bool> OneMessage; //sjkang
...
private:
 ...
 // std::vector<uint16_t> SN;
  //Time ArrivalTime;
  std::map<uint32_t, OneMessage> m_Messages;
  std::map <uint32_t, bool> m_SuccessIndication;
  std::map<uint32_t, Time> m_ArrivalTime;
  /**
