...
main (int argc, char *argv[])
{
	...
	uint16_t typeOfDuplication = 0; //gsoul 180905
	...
	Config::SetDefault("ns3::McEnbPdcp::DataDuplicationMetrics",UintegerValue(typeOfDuplication));
	...
	for (uint16_t i = 0; i<ueNodes.GetN(); i++) {
		if (i == 0)
			uePositionAlloc->Add(Vector(0, 15, 1.5));
		else if (i == 1)
			uePositionAlloc->Add(Vector(0,-15, 1.5));
		else if (i == 2)
...
//	uemobility.SetMobilityModel("ns3::ConstantPositionMobilityModel");
	...
	building1->SetBoundaries(Box(-8,8,
				5, 7.5,
				0.0, 15));
/*
...
*/	/*
	 ...
	 */				                       /*
	 ...
	 Simulator::Schedule(Seconds(0.2),&ChangeSpeed, ueNodes.Get(0),Vector(2,0,0));
	...
	if (tcp){
		...
		for (uint32_t u = 0; u < ueNodes.GetN (); ++u)
		{
			if(dl)
			{
				...
				Address sinkAddress (InetSocketAddress (ueIpIface.GetAddress (u), dlPort));
				app->Setup (ns3UdpSocket, sinkAddress, 1400, nPacket, DataRate ("1Mbps"),isRandom);
				remoteHostContainer.Get (u)->AddApplication (app);
