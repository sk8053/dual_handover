...
void
PrintLostUdpPackets(Ptr<OutputStreamWrapper> stream, Ptr<UdpServer> app)
{	
	if(app->GetReceived()!=0){
		*stream->GetStream() << Simulator::Now().GetSeconds() << " Lost: " << (app->GetLost()) <<" Received: "<<(app->GetReceived()) <<std::endl;
	}
	
	Simulator::Schedule(MilliSeconds(20), &PrintLostUdpPackets,stream,app);
}

void
PrintSentUdpPackets(Ptr<OutputStreamWrapper> stream, Ptr<UdpClient> app)
{
	*stream->GetStream() << Simulator::Now().GetSeconds() <<" Sent: "<<(app->GetSent())<<std::endl;
	Simulator::Schedule(MilliSeconds(20), &PrintSentUdpPackets,stream,app);
}
	static void
...
//	uint32_t nPacket = 500000000;
	...
	DataRate udpRate =DataRate("100Mbps");
...
//	mmwaveHelper->SetAttribute("ChannelModel", StringValue("ns3::MmWave3gppChannel"));
	...
	Vector mmw1Position = Vector(100, 0, 10);  ///28Ghz 
	Vector mmw2Position = Vector(-100, 0, 10); //28Ghz
...
/*	for (uint16_t i = 0; i<ueNodes.GetN(); i++) {
	...
	for (uint16_t i = 0;  i<row; i++){
		for (uint16_t j = 0; j<col; j++){
			uePositionAlloc -> Add (Vector (-200+100*j,200-100*i,1.5));
		}
	}
...
*/	/*
	...
	NetDeviceContainer mmWaveEnbDevs_28GHZ = mmwaveHelper->InstallEnbDevice_28GHZ(mmWaveEnbNodes.Get(0));
	NetDeviceContainer mmWaveEnbDevs_28GHZ_2 = mmwaveHelper->InstallEnbDevice_28GHZ(mmWaveEnbNodes.Get(1));
	NetDeviceContainer mmWaveEnbDevs_73GHZ = mmwaveHelper->InstallEnbDevice_73GHZ(mmWaveEnbNodes.Get(2));
	...
	if (tcp){
		...
		for (uint32_t u = 0; u < ueNodes.GetN (); ++u)
		{
			...
			//	PacketSinkHelper dlPacketSinkHelper ("ns3::UdpSocketFactory", InetSocketAddress (Ipv4Address::GetAny (), dlPort));
				...
				dlUdpServerHelper.SetAttribute("PacketWindowSize",UintegerValue(1024));	
			...
			/*	
				...
				Address sinkAddress (InetSocketAddress (ueIpIface.GetAddress (u), dlPort));
				app->Setup (ns3UdpSocket, sinkAddress, 32, nPacket, DataRate ("100Mbps"),isRandom);
				remoteHostContainer.Get (u)->AddApplication (app);
			*/
				std::ostringstream fileName;
				fileName<<"packet_loss_rate_ue_"<<u+1<<".txt";
				AsciiTraceHelper asciiTraceHelper;
				Ptr<OutputStreamWrapper> stream = asciiTraceHelper.CreateFileStream(fileName.str().c_str());
				Simulator::Schedule(MilliSeconds(100), &PrintLostUdpPackets,stream,DynamicCast<UdpServer>(serverApps.Get(u)));

				UdpClientHelper dlClient (ueIpIface.GetAddress (u), dlPort);
				dlClient.SetAttribute("Interval", TimeValue(Seconds(interPacketInterval)));
				dlClient.SetAttribute("MaxPackets", UintegerValue(nPacket));
				dlClient.SetAttribute("PacketSize",UintegerValue(32));
				clientApps.Add (dlClient.Install(remoteHostContainer.Get(u)));
				
				std::ostringstream fileName2;
				fileName<<"packet_send_ue"<<u+1<<".txt";
				AsciiTraceHelper asciiTraceHelper2;
				Ptr<OutputStreamWrapper> stream2 = asciiTraceHelper2.CreateFileStream (fileName2.str ().c_str ());
				Simulator::Schedule(MilliSeconds(100), &PrintSentUdpPackets,stream2,DynamicCast<UdpClient>(clientApps.Get(u)));
				/*
				std::ostringstream fileName_2;
