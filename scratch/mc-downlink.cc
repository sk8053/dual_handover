...
main (int argc, char *argv[])
{
	...
	double TxPower = 23;
	uint16_t typeOfSplitting = 5; // 3 : p-split
	...
	//Scenario: High speed train with 4 mmw gnbs and 1 lte enb
	...
	mmWaveEnb28GhzNodes.Create(2);
	mmWaveEnb73GhzNodes.Create(2);
	mmWaveEnbNodes.Add(mmWaveEnb28GhzNodes);
	mmWaveEnbNodes.Add(mmWaveEnb73GhzNodes);
	...
	//  Vector mmw1Position = Vector(100.0,0, 1);  ///73Ghz //path 1
	//   Vector mmw2Position = Vector(-100.0, 0.0, 1); //28Ghz // path 0
	...
	if (tcp){
		...
		for (uint32_t u = 0; u < ueNodes.GetN (); ++u)
		{
			if (dl)
			{
				...
				app->Setup (ns3TcpSocket, sinkAddress, 1400, 5000000, DataRate ("1500Mbps"));
		...
		for (uint32_t u = 0; u < ueNodes.GetN (); ++u)
		{
			if(dl)
			{
				...
				Address sinkAddress (InetSocketAddress (ueIpIface.GetAddress (u), dlPort));
				app->Setup (ns3UdpSocket, sinkAddress, 1400, nPacket, DataRate ("2000Mbps"));
				remoteHostContainer.Get (u)->AddApplication (app);
