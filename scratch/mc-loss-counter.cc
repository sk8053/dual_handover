...
CalculateThroughput (Ptr<OutputStreamWrapper> stream, Ptr<PacketSink> sink, uint16_t i)
{
	...
	double cur = (sink->GetTotalRx () - lastTotalRx[i]) * (double) 8 / 1e4;     /* Convert Application RX Packets to MBits. */
	c[i]++;
	t_total[i]+=cur;
...
static void
Loss (Ptr<OutputStreamWrapper> stream , uint64_t received, uint32_t lossAmount, double delay ){
	*stream->GetStream() << Simulator::Now().GetSeconds() << "\t"<< lossAmount<< "\t" << received <<"\t"<<delay<<std::endl;
}
	void
...
main (int argc, char *argv[])
{
	...
	bool harqEnabled = true;//HARQ is needed to analyze large cluster of packet loss
	bool rlcAmEnabled = false;
	bool fixedTti = false;
	unsigned symPerSf = 24;
	double sfPeriod = 100.0;
	bool tcp = true, dl= true, ul=false;
	double x2Latency = 1000, mmeLatency=0.0;
	double TxPower = 30;
...
//	bool isRandom = false; //gsoul 180910 for random traffic generate
	...
	//Sceanario: BS, UE, buildings deployment	
	...
	NodeContainer mmWaveEnb28GhzNodes;
	NodeContainer mmWaveEnb73GhzNodes;
	NodeContainer lteEnbNodes;
	NodeContainer allEnbNodes;
//	mmWaveEnbNodes.Create(2);
	mmWaveEnb28GhzNodes.Create(1);
	mmWaveEnb73GhzNodes.Create(1);
	lteEnbNodes.Create(1);
	ueNodes.Create(nodeNum);
	mmWaveEnbNodes.Add(mmWaveEnb28GhzNodes);//gsoul 190102
	mmWaveEnbNodes.Add(mmWaveEnb73GhzNodes);//gsoul 190102
	allEnbNodes.Add(lteEnbNodes);
	allEnbNodes.Add(mmWaveEnbNodes);

	Vector LTEPosition = Vector(0, 20, 35); //LTE eNB		
	Vector mmw1Position = Vector(580, -8, 35);  //28Ghz_1 
	Vector mmw2Position = Vector(1632, -8, 35); //28Ghz_2
	Vector mmw1_2Position = Vector(0,8,36); //73Ghz_1
	Vector mmw2_2Position = Vector(1060,8,35); //73Ghz_2
//	Vector mmw3Position = Vector(0,0,10);//73Ghz not used
...
//	enbPositionAlloc->Add(mmw3Position);
	...
	for (uint16_t i = 0;  i<row; i++){
		for (uint16_t j = 0; j<col; j++){
			uePositionAlloc -> Add (Vector (0,0,10));
		}
	}
...
//        ueNodes.Get (0)->GetObject<MobilityModel> ()->SetPosition (Vector (30, 0, 0));
	...
	NetDeviceContainer mmWaveEnbDevs_28GHZ = mmwaveHelper->InstallEnbDevice_28GHZ(mmWaveEnb28GhzNodes);
	NetDeviceContainer mmWaveEnbDevs_73GHZ = mmwaveHelper->InstallEnbDevice_73GHZ(mmWaveEnb73GhzNodes);
//	NetDeviceContainer mmWaveEnbDevs_28GHZ_2 = mmwaveHelper->InstallEnbDevice_28GHZ(mmWaveEnbNodes.Get(1));

//	mmWaveEnbDevs_28GHZ.Add(mmWaveEnbDevs_28GHZ_2.Get(0));
	...
	if (tcp){
		...
		for (uint32_t u = 0; u < ueNodes.GetN (); ++u)
		{
			if (dl)
			{
				...
				app->Setup (ns3TcpSocket, sinkAddress, 1400, 5000000, DataRate ("1Gbps"));
				...
				Simulator::Schedule (Seconds (0.2001), &Traces, u);
		...
		for (uint32_t u = 0; u < ueNodes.GetN (); ++u)
		{
			if(dl)
			{
				...
				std::ostringstream fileName_loss;
				fileName_loss<<"packet_loss"<<u+1<<".txt";
				AsciiTraceHelper asciiTraceHelper3;
				Ptr<OutputStreamWrapper> stream_loss = asciiTraceHelper3.CreateFileStream (fileName_loss.str().c_str());
				serverApps.Get(u)->TraceConnectWithoutContext ("Loss", MakeBoundCallback (&Loss, stream_loss));

				Ptr<PacketSink> sink0 = serverApps.Get(u)->GetObject<PacketSink>();
				sink0->SetAttribute("MessgeTransmission", BooleanValue(true));
				sink0->SetAttribute("PacketNumber",UintegerValue(nPacket));
				app->Setup (ns3UdpSocket, sinkAddress, 1400, nPacket, udpRate);
	...
	// Start applications
	Config::Set ("/NodeList/*/DeviceList/*/TxQueue/MaxPackets", UintegerValue (0XFFFFFFFF));
...
//	flowMonitor -> SerializeToXmlFile("FlowOfManyNodes.xml",true,true);
